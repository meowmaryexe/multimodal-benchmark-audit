...
OUTPUT_PATH = "results/plotqa/internvl2_4b_plotqa_test.csv"
...
def categorize_question(question):
    ...
    compositional_keywords = [
        "difference", "average", "ratio", "sum", "total",
        "how many more", "how many fewer", "greater than",
...
def load_completed(output_path):
    ...
    with open(output_path, "r", newline="") as f:
        ...
        for row in reader:
            completed.add((int(row["chart_index"]), int(row["qa_index"]), row["condition"]))
    return completed
...
def image_to_tensor(image, input_size=448):
    transform = build_transform(input_size=input_size)
    pixel_values = transform(image).unsqueeze(0)
    return pixel_values.to(dtype=torch.float16, device="cuda")
...
fieldnames = [
    "chart_index", "qa_index", "condition", "category",
    "question", "gold", "pred", "correct",
]
