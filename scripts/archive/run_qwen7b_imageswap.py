# Run Qwen2-VL-7B on ChartQA with WRONG IMAGE counterfactual
...
correct = 0
total = 200   # image-swap robustness check
...
for i in range(total):
    ...
    # WRONG IMAGE: use the next example's chart instead of the correct one
    wrong_image = dataset[(i + 1) % len(dataset)]["image"]

    messages = [
        {
            "role": "user",
            "content": [
                {"type": "image", "image": wrong_image},
                {
                    "type": "text",
